...
bool SqliteNoteRepository::createNote(
    const QString &title,
    const QString &body,
    Note *createdNote)
{
    ...
    if (!query.exec()) {
        m_lastError =
            QStringLiteral("Failed to save note: %1")
    ...
    if (createdNote) {
        createdNote->id = query.lastInsertId().toInt();
        createdNote->title = title;
        createdNote->body = body;
        createdNote->createdAt = timestamp;
        createdNote->updatedAt = timestamp;
    }

    return true;
}

bool SqliteNoteRepository::updateNote(
    int id,
    const QString &title,
    const QString &body,
    Note *updatedNote)
{
    ...
    if (!query.exec()) {
        m_lastError =
            QStringLiteral("Failed to update note: %1")
                .arg(query.lastError().text());
    ...
    if (query.numRowsAffected() != 1) {
        m_lastError = QStringLiteral("Note was not found");
        return false;
    }
    ...
    if (updatedNote) {
        ...
        if (!findNote(id, &note)) {
            m_lastError =
                QStringLiteral("Note was updated but could not be reloaded");
            return false;
        }
...
bool SqliteNoteRepository::deleteNote(int id)
{
    ...
    if (!query.exec()) {
        m_lastError =
            QStringLiteral("Failed to delete note: %1")
                .arg(query.lastError().text());
...
bool SqliteNoteRepository::findNote(
    int id,
    Note *note) const
{
    m_lastError.clear();
    ...
    if (!query.exec()) {
        m_lastError =
            QStringLiteral("Failed to read note: %1")
                .arg(query.lastError().text());
...
QList<Note> SqliteNoteRepository::listNotes() const
{
    ...
    if (!query.exec(QStringLiteral(
        ...
        m_lastError =
            QStringLiteral("Failed to list notes: %1")
                .arg(query.lastError().text());
...
int SqliteNoteRepository::count() const
{
    ...
    if (!query.exec(QStringLiteral(
        ...
        m_lastError =
            QStringLiteral("Failed to count notes: %1")
                .arg(query.lastError().text());
