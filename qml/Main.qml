...
Window {
    ...
    // Ubuntu Touch/Lomiri owns the actual application canvas.
    // Do not derive the application size from desktopAvailableWidth/Height.
    // Those values are not the right abstraction for a mobile fullscreen app.
    visibility: Window.FullScreen

    // Fallback size for desktop/offscreen testing.
    width: 360
    height: 760
    ...
    function automationTick() {
        ...
        case 4: {
            ...
            if (noteController.noteCount >
                window.__automationInitialCount + 1) {

                automationFail(
                    "count=" + noteController.noteCount
                )
                return
            }

            // Wait for the asynchronous save.
            automationTimer.ticks--
            break
        }
        ...
        default:
            if (automationTimer.ticks > 40) {
                automationFail(
                    "timeout, tick=" + automationTimer.ticks
                )
            }
        }
    }
