...
Version **0.2.2** extends the basic create/read/update workflow with permanent deletion and a dedicated mobile UX scaling system.
...
The current 0.2.2 UX refinement also keeps the delete-confirmation dialog wider without increasing the scale of the controls inside it.
...
The application remains on **0.2.2** for these UI refinements. The version should not be incremented merely for the delete-dialog styling changes.

---

# Current 0.2.2 UX behavior
...
Only the dialog surface width/padding is enlarged; the internal control sizing remains governed by the existing 0.2.2 UI metrics.
